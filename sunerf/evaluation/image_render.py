...
class ImageRender:
    ...
    def save_frame_as_fits(self, i, point, model_outputs, wavelength, half_fov=1.3, itype='imager', obs_date='2014-04-01T00:00:00.000'):
        ...
        output_path =  f"{self.render_path}/{itype}/{wavelength}"
        os.makedirs(output_path, exist_ok=True)
        img_path = f'{output_path}/{str(i).zfill(3)}_w{wavelength}_lat{np.round(lat,1)}_lon{np.round(lon,1)}_r{np.round(d,2)}_T{np.round(time,2)}.fits'

        if not os.path.exists(img_path):

             # Undo normalization
            predicted = outputs['pixel_intensity']
