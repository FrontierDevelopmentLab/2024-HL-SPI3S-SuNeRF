...
class NonUniformStratifiedSampler(torch.nn.Module):

    def __init__(self, Rs_per_ds, distance=1.3, n_samples=64,
                 grid_exponent= 7.0, perturb=True):
        super(NonUniformStratifiedSampler, self).__init__()
        self.perturb = perturb

        self.register_buffer('grid_exponent', torch.tensor(grid_exponent, dtype=torch.float32))
        self.register_buffer('distance', torch.tensor(distance / Rs_per_ds, dtype=torch.float32))
        self.register_buffer('solar_R', torch.tensor(1 / Rs_per_ds, dtype=torch.float32))
    ...
    def forward(self, rays_o: torch.Tensor, rays_d: torch.Tensor):
        ...
        new_t_vals = radius.pow(self.grid_exponent)
        ...
        z_vals = dist_near[:, None] * (1. - new_t_vals) + dist_far[:, None] * (new_t_vals)
